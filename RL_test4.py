...
class ActorCritic:
    ...
    def create_critic_model(self):
        ...
        action_h1 = Dense(1000)(action_input)
...
def main():
    ...
    for i in range(num_trials):
        ...
        delta_vector = []
        bs_delta_vector = []
        cumulative_reward_vector = []
        bs_cumulative_reward_vector = []
        cumulative_reward = 0
        ...
        while done == False:
            ...
            #en# v.render() (copied from RL_test3)

            #print('reward', reward)
            #print('bs_reward', bs_reward)

            #reward += reward
            #bs_reward += bs_reward

            price_vector.append(new_state[0])
            delta_vector.append(delta[0])
            bs_delta_vector.append(bs_delta)
            cumulative_reward += reward[0]
            bs_cumulative_reward += bs_reward

            cumulative_reward_vector.append(cumulative_reward)
            bs_cumulative_reward_vector.append(bs_cumulative_reward)
        ...
        if i%1==0:
            dict = {'iteration': i, 'price_vector': price_vector, 'delta_vector': delta_vector,
                    'bs_delta_vector': bs_delta_vector, 'cumulative_reward' : cumulative_reward_vector,
                    'bs_cumulative_reward' : bs_cumulative_reward_vector}
